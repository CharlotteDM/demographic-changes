...
`r EU_HighFertRat$Country.Name`. The fertility rate is 
`r EU_HighFertRat$eu_hfr`.
...
`r EU_LowFertRat$Country.Name`. The fertility rate is 
`r EU_LowFertRat$eu_lfr`.

This map show fertility rate in EU countries in 2019. 
...
This chart show fertility rate in EU countries in 2019.

```{r EU_FR=plot, dev='png', echo=FALSE}
plot(EU_FR)
```

__Death Rate__ (crude, per 1000 people) is: 
